...
const Hero = () => {
  ...
  const handleDownloadClick = () => {
    ...
    const isImage = /\.(jpe?g|png|gif)$/i.test("/cv.jpg");
    const filePath = isImage ? "/cv.jpg" : "/cv.pdf";
  
    // Use window.open for better cross-browser support
    const newWindow = window.open(filePath, '_blank');
    
    if (newWindow) {
      // If the window opened, change status to downloaded
      setDownloadStatus("Downloaded");
      setTimeout(() => setDownloadStatus("Download CV"), 3000);
    } else {
      // If the window did not open (e.g., popup blocker), show error
