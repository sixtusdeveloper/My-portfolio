    ...
    <section id="Services" className="relative md:pt-10 mx-auto max-w-7xl px-6 lg:px-16 pb-20">
      ...
      {isLoading && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
          <div className="loader"></div> {/* Loader component or loading animation */}
        </div>
      )}
...
//   const [isModalOpen, setIsModalOpen] = useState(false);
//   const [selectedService, setSelectedService] = useState<Service | null>(null);
//   const [loadingServiceId, setLoadingServiceId] = useState<number | null>(null); // State to track loading button
//   const modalRef = useRef<HTMLDivElement | null>(null);
//   const scrollPosition = useRef(0);

//   const openModal = (serviceItem: Service, index: number) => {
//     setLoadingServiceId(index); // Set the loading state for the specific button
//     setSelectedService(serviceItem);
...
//       scrollPosition.current = window.scrollY;
//       setIsModalOpen(true);
//       setLoadingServiceId(null); // Stop loading
//     }, 2000); // Adjust the delay as needed
//   };
...
//             borderRadius="1.75rem"
//             duration={Math.floor(Math.random() * 10000) + 10000}
//             className={`flex-1 text-white border-neutral-200 dark:border-slate-800 ${loadingServiceId === index ? 'opacity-50 cursor-not-allowed' : ''}`}
//             onClick={() => openModal(service, index)}
//             disabled={loadingServiceId === index} // Disable the button while loading
//           >
//             {loadingServiceId === index ? (
